...
> **最后更新**：2026-07-29（**ML-10 特征 vs 目标 Pearson cell 精读**：读透 cell[26]——feat_cols 拼接 23 特征 + `.corrwith(log_wear)` + 红蓝 barh；核心收获是**线性尺子的两个盲区**：漏掉非单调（binder U 型隐身 r≈0）、看不见交互项（7 号机理在图上完全不存在）。关键区分：Pearson 抓不抓得到取决于**单调性**不是曲直。见 §5.5 与 §4 教训 18。纯读代码会话，无代码 commit，仅本文档更新）
...
- **NAO摩擦材料ML-10-特征vs目标Pearson cell精读(线性尺子盲区:binder U型隐身r≈0/漏交互项, corrwith分水岭, if头尾之别)（07-29，本次）**
- （下次）NAO摩擦材料ML-11-…（二选一，见 §9）
...
> 📌 **精读进度**：块 1 + EDA 分布图 cell + 相关矩阵 cell + **特征 vs 目标 Pearson cell（ML-10）** 均已读完。**下一个 cell 就是真正的 `train_test_split`（主干块 2）**——ML-10 因「不跳过」原则先读了排在它前面的 Pearson cell，块 2 顺延到这次。

**选项 A —— 走支线（接着精读代码，07-25 定的默认路线）**

> 「继续这个项目（ML-11）。ML-10 读完了特征 vs 目标 Pearson cell（线性尺子两盲区：binder U 型隐身、漏交互项）。今天回到代码精读：读**块 2 切分训练/测试集**那个 cell（含 `train_test_split`、X/y 约定、random_state=42）。老规矩：一次一块、块内逐行、四问模板，我先说理解你再拧。」

**选项 B —— 走主线（阶段 3，学术亮点）**

> 「继续这个项目（ML-11）。阶段 2 已完结（7/7 机理找回），GitHub 双机同步已落地，代码精读已过完块 1 + EDA 分布图 cell + 相关矩阵 cell（闭合铁证 r=−1.000）+ 特征 vs 目标 Pearson cell（线性尺子盲区）。今天开阶段 3：成分数据的对数比变换。先给我讲清楚**闭合数据为什么需要 CLR/ILR**、它到底治好了什么病，我听懂了再动手写代码。」
