...
> **最后更新**：2026-07-30（**ML-11 binder 二次拟合 cell 精读**：读透 cell[27]——用**二次拟合**给 ML-10「线性尺子在 U 型上失灵」当**当庭解药**：①两个 Pearson r 实证「合体 binder_total 也救不了，r≈0」②`polyfit/linspace/polyval` 三步造平滑拟合曲线③`truth` 真值绿线**加平移常数**抬到数据高度带同台比对④画图 + `-b/(2a)` 算出拟合谷底 **13.25（真值 13.00）**。三个高含金量追问理清一条深理：**加性机理模型里单项绝对高度无意义、只有形状有意义，高度由截距常数统一承担**。另附 LaTeX 画图速查（化学式下标 / μ）。见 §5.5、§4 教训 19、§5.6。纯读代码会话，无代码 commit，仅本文档更新）
...
| … | 切分训练/测试集（含 `train_test_split`） | ⬜ **下一站，待读** | 主干块 2 |
...
- [ ] 块 2 切分训练/测试集（含 `train_test_split`）← **下一站**
...
- **NAO摩擦材料ML-11-binder二次拟合cell精读(线性盲区解药:polyfit求谷底13.25 vs真值13.00, 单项高度无意义只形状有意义, 真值绿线平移对齐, LaTeX画图速查)（07-30，本次）**
- （下次）NAO摩擦材料ML-12-…（二选一，见 §9）
...
> 📌 **精读进度**：块 1 + EDA 分布图 cell + 相关矩阵 cell + 特征 vs 目标 Pearson cell + **binder 二次拟合 cell（ML-11）** 均已读完。**下一个 cell 就是真正的 `train_test_split`（主干块 2）**——ML-11 因「不跳过」原则先读了排在它前面的二次拟合 cell（`[27]`），块 2 顺延到下次。

**选项 A —— 走支线（接着精读代码，07-25 定的默认路线）**

> 「继续这个项目（ML-12）。ML-11 读完了 binder 二次拟合 cell（线性盲区的解药：polyfit 求谷底 13.25、单项高度无意义只形状有意义）。今天回到代码精读：读**块 2 切分训练/测试集**那个 cell（含 `train_test_split`、X/y 约定、random_state=42）。老规矩：一次一块、块内逐行、四问（3W1H）模板，我先说理解你再拧。」

**选项 B —— 走主线（阶段 3，学术亮点）**

> 「继续这个项目（ML-12）。阶段 2 已完结（7/7 机理找回），GitHub 双机同步已落地，代码精读已过完块 1 + EDA 分布图 cell + 相关矩阵 cell（闭合铁证 r=−1.000）+ 特征 vs 目标 Pearson cell + binder 二次拟合 cell（谷底 13.25）。今天开阶段 3：成分数据的对数比变换。先给我讲清楚**闭合数据为什么需要 CLR/ILR**、它到底治好了什么病，我听懂了再动手写代码。」

**建议**：两条线不冲突。阶段 3 是论文学术亮点，优先级更高；代码精读可在阶段 3 每次开工前当「热身」穿插做一个 cell。按 §0.1，精读现在是「每 cell 都过」，进度会比原来慢些但更扎实。**下次就是主干块 2（`train_test_split`）——精读到这里已摸到建模主干门口了。**
